...
/*## 관리자 타회원아이디 주문시 ################################################*/
	if ($member['mb_level'] > 8 && $od['mb_id'] != $member['mb_id']) {
		alert("주문이 성공적으로 이루어졌습니다. 주문조회는 관리자 화면에서 관리 가능합니다.", G5_SHOP_URL);
	} else {
    ...
    <div id="sod_fin_view">
        ...
        <section id="sod_fin_pay">
            ...
            <div class="tbl_head01 tbl_wrap">
				...
				<? if ($od['od_receipt_cash'] > 1) { ?>
					<? if ($od_receipt_price != "아직 입금되지 않았거나 입금정보를 입력하지 못하였습니다.") { ?>
                <tr>
                    <th scope="row"><?php echo ($easy_pay_name ? $easy_pay_name.'('.$od['od_settle_case'].')' : $od['od_settle_case']); ?></th>
                    <td>
						...
						<? } ?>
                <tr>
                    <th scope="row">현금결제</th>
                    <td><?=number_format($od['od_receipt_cash'])?>원</td>
                </tr>
				<? } else { ?>
                <tr>
                ...
                // 계좌정보
                if($disp_bank && $od['od_settle_case'] != "매장카드")
                {
                ?>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if($od['od_settle_case'] == '휴대폰')
                        {
                            ...
                            } else if($od['od_pg'] == 'billgate') {
                                //0900 신용카드, 휴대폰 : 1100, 계좌이체 : 1000
                                $receipt_url = "http://tcp.billgate.net/billgate/common/commonReceipt.jsp?mid=".$MERCHANT_ID."&transNm=".$od['od_tno']."&currTp=1100";
                               // $receipt_url = "https://cpadmin.billgate.net/billgate/common/authCardReceipt.jsp?mid=".$MERCHANT_ID."&transNm=".$od['od_tno']."&currTp=0000";
                                $hp_receipt_script = 'window.open(\''.$receipt_url.'\',\'receipt\',\'width=400,height=750\');';
                            } else {
                        ...
                        ?>
                        <a href="javascript:;" onclick="<?php echo $hp_receipt_script; ?>">거래확인서 출력</a>
                        <?php
                        }
                        ...
                        if($od['od_settle_case'] == '신용카드')
                        {
                            if($od['od_pg'] == 'lg') {
                                ...
                                $receipt_url = "http://tcp.billgate.net/billgate/common/commonReceipt.jsp?mid=".$MERCHANT_ID."&transNm=".$od['od_tno']."&currTp=0000";
                               // $receipt_url = "https://cpadmin.billgate.net/billgate/common/authCardReceipt.jsp?mid=".$MERCHANT_ID."&transNm=".$od['od_tno']."&currTp=0000";
                                $card_receipt_script = 'window.open(\''.$receipt_url.'\',\'receipt\',\'width=400,height=750\');';
                            } else {
                        ...
                        ?>
                        <a href="javascript:;" onclick="<?php echo $card_receipt_script; ?>">거래확인서 출력</a>
                        <?php
                        }
