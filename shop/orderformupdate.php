...
/*## 현금, 매장카드 추가 ################################################*/
/*## 현금, 매장카드 추가 ################################################*/
## $member['mb_id'] 를 모두다 $mb_id로 변경함
if (trim($_POST['set_id_ipt'])) {
	$mb_id = trim($_POST['set_id_ipt']);
} else if (trim($_POST['set_id_sel'])) {
	$mb_id = trim($_POST['set_id_sel']);
} else {
	$mb_id = $member['mb_id'];
}
/*@@End. 현금, 매장카드 추가 #####*/
...
if($is_member) {
    ...
    if($_POST['od_cp_id']) {
        ...
        // 사용한 쿠폰인지
        $cp_used = is_used_coupon($mb_id, $cp['cp_id']);
...
/*## 추가된 현금로직(포인트 재활용).2-1 ################################################*/
/*## 추가된 현금로직(포인트 재활용).2-1 ################################################*/
//$_POST['od_temp_cash'];
$od_temp_cash = str_replace(",","",trim($_POST['od_temp_cash']));
/*@@End. 추가된 현금로직(포인트 재활용) #####*/
/*@@End. 추가된 현금로직(포인트 재활용) #####*/
...
/*## 현금, 매장카드 추가 ################################################*/
/*## 현금, 매장카드 추가 ################################################*/
if ($od_settle_case == "무통장" || $od_settle_case == "현금" || $od_settle_case == "매장카드")
{
/*
...
else if ($od_settle_case == "계좌이체")
{
    switch($default['de_pg_service']) {
        ...
        case 'inicis':
            ...
            break;
        case 'billgate':
            include G5_SHOP_PATH.'/billgate/billgate_result.php';
            break;
        default:
    ...
    if((int)$order_price !== (int)$pg_price) {
        ...
        switch($od_pg) {
            ...
            case 'inicis':
                ...
                break;
            case 'billgate':
                include G5_SHOP_PATH.'/billgate/billgate_cancel.php';
                break;
            case 'KAKAOPAY':
                $_REQUEST['TID']               = $tno;
                $_REQUEST['Amt']               = $amount;
...
if ($is_member && $od_receipt_point)
    insert_point($mb_id, (-1) * $od_receipt_point, "주문번호 $od_id 결제");
...
if($config['cf_sms_use'] && ($default['de_sms_use2'] || $default['de_sms_use3'])) {
    ...
    if($is_sms_send) {
        ...
        for($s=0; $s<count($sms_contents); $s++) {
            ...
            $sms_content = str_replace("{회원아이디}", $mb_id, $sms_content);
...
/*## 타회원아이디 주문 후 cart 정보 변경 ################################################*/
/*## 타회원아이디 주문 후 cart 정보 변경 ################################################*/

	if ($od_settle_case == "현금" || $od_settle_case == "매장카드") {
			/*## 추가된 현금로직(포인트 재활용).2-2 ################################################*/
			/*## 추가된 현금로직(포인트 재활용).2-2 ################################################*/
				$tot_ct_price = $tot_ct_price - $od_temp_cash;
			/*@@End. 추가된 현금로직(포인트 재활용) #####*/
			/*@@End. 추가된 현금로직(포인트 재활용) #####*/

		sql_query(" update g5_shop_order set od_status = '입금', od_receipt_price = '{$tot_ct_price}', od_receipt_time = now(), od_misu = '0' where od_id = '{$od_id}' "); // 주문하자마자 입금인증
		sql_query(" update g5_shop_cart set ct_status = '입금' where od_id = '{$od_id}' ");
	}

	// 주문자 카트 아이디 변경
	sql_query(" update g5_shop_cart set mb_id = '{$mb_id}' where od_id = '{$od_id}' ");

/*@@End. 타회원아이디 주문 후 cart 정보 변경 #####*/
/*@@End. 타회원아이디 주문 후 cart 정보 변경 #####*/
...
goto_url('/new/invoice.php?od_id='.$od_id.'&uid='.$uid.'&payMethod='.$_POST['payMethod']);
?>
