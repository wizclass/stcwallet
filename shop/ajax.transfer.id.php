...
<div class="infoBx">
	<h3>Transfer Id Search</h3>
	<ul>
	<?
		$i = 0;
		$qry = sql_query(" select mb_id, mb_name from g5_member where mb_leave_date = '' and (mb_id like '%{$_GET[mbid]}%' or mb_name like '%{$_GET[mbid]}%')  order by mb_id ");
...
$(function(){
	$('span[id^="id_"]').click(function () {
		var $id = $(this).attr("id").replace("id_","");
		$("#recipient, #recipient",parent.document.body).val($id);
		$("#recipient",parent.document.body).focus();
		$("#framer",parent.document.body).attr("src","");
		$("#framewrp",parent.document.body).hide();
...
<div class="infoBx">
	...
	<?
		$i = 0;
		$qry = sql_query(" select mb_id, mb_name from g5_member where mb_leave_date = '' and mb_id != '{$_GET['mb_id']}' and (mb_id like '%{$_GET[rcm]}%' or mb_name like '%{$_GET[rcm]}%') and mb_level > 0 order by mb_id ");
		while ($res = sql_fetch_array($qry)) {
			if ($res['mb_id']) {
	?>
		<li><span id="id_<?=$res['mb_id']?>"><?=$res['mb_id']?><p>(<?=$res['mb_name']?>)</p></span></li>	
