...
function print_item_options($it_id, $cart_id)
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if($row['io_price'] >= 0)
            $price_plus = '+';
        $str .= '<li>'.get_text($row['ct_option']).' '.$row['ct_qty'].'EA</li>'.PHP_EOL;
    }
...
function save_order_point($ct_status="완료")
{
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        // 회원 ID 를 얻는다.
        $od_row = sql_fetch("select od_id, mb_id, pv, bv from {$g5['g5_shop_order_table']} where od_id = '{$row['od_id']}' ");
		//## 기존
...
function insert_pvbv($od_id) {
	//echo $default['it_point_type'];

	$pv = $bv = 0;

	$qry = sql_query(" select it_id, ct_qty from g5_shop_cart where od_id = '{$od_id}'  ");
	while ($res = sql_fetch_array($qry)) {
		$it = "";
		$it = sql_fetch(" select it_price, it_point_type, it_point, it_bv from g5_shop_item where it_id = '{$res[it_id]}' ");
		if ($it && $it['it_point_type'] > 0) {
			$pv = $pv + (($it['it_price'] * $it['it_point'] / 100) * $res['ct_qty']);
		} else {
			$pv = $pv + ($it['it_point'] * $res['ct_qty']);
		}

		$bv = $bv + ($it['it_bv'] * $res['ct_qty']);
	}
	sql_query(" update g5_shop_order set pv = '{$pv}', bv = '{$bv}' where od_id = '{$od_id}' ");
}
//==============================================================================
// 쇼핑몰 라이브러리 모음 끝
//==============================================================================
