...
// 이름검색
$stx = urldecode($_GET['stx']);
if ($stx) {
	if($sfl != "all"){$sql_search .= " and ";}
...
$mining_sql = "from soodang_mining where (1)";
$sql = "SELECT mb_id,mb_name,allowance_name,day,mining AS benefit,currency,rec,rec_adm,datetime {$mining_sql} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";

$result = sql_query($sql);
$listall = "<a href='{$_SERVER['PHP_SELF']}?start_dt={$fr_date}&end_dt={$to_date}&view_mode=all' class='ov_listall' >전체목록</a>";
...
<div class="local_desc01 local_desc">
    <p>
		<strong>스테이킹 보너스 :</strong> 스테이킹 상품관리에 입력된 데이터 기준으로 지급 됩니다. <br>
	</p>
</div>
...
<form name="fsearch" id="fsearch" class="local_sch01 local_sch" style="clear:both;padding:10px 20px 20px;" method="get" onsubmit="return encode_url(this);">
	
			<label for="sfl" class="sound_only">검색대상</label>
			<select name="sfl" id="sfl" onchange="get_select_value();">
				...
				<option value="allowance_name"<?php echo get_selected($sfl, "allowance_name"); ?>>보너스이름</option>
				<option value="currency"<?php echo get_selected($sfl, "currency"); ?>>보너스단위</option>
			</select>
			...
			<input type="button" class="btn_submit excel" id="btnExport"  data-name='staking_history' value="엑셀 다운로드" />
...
function get_select_value(){
	let input_stx = document.querySelector('#stx');
	let select_sfl = document.querySelector('#sfl');

	input_stx.disabled = false;

	if(select_sfl.value == "all"){
		input_stx.value = "";
		input_stx.disabled = true;
	}
}

function encode_url(f){
	f.stx.value = f.stx.value.replace(/&/g,"%26").replace(/\+/g,"%2B").replace(/=/g,"%3D");
	window.location.href=`./bonus_staking.php?sfl=${f.sfl.value}&stx=${encodeURIComponent(f.stx.value)}&fr_date=${f.start_dt.value}&to_date=${f.end_dt.value}`;
	return false;
}
</script>
