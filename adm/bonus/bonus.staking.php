...
if(!$debug){
    $dupl_check_sql = "select mb_id from soodang_mining where day='{$bonus_day}'";
    $get_today = sql_fetch($dupl_check_sql);

    if($get_today['mb_id']){
        alert($bonus_day.' '.$code." 은 이미 지급되었습니다.");
        die;
    }
}

$pre_sql = "select m.mb_id, m.mb_level, s.* from {$g5['member_table']} m join {$g5['g5_shop_order_table']} s on m.mb_no = s.mb_no where 
s.od_hope_date = '{$bonus_day}' and 
...
echo "<span class ='title' style='font-size:20px;'>".$bonus_row['name']." 스테이킹 정산</span><br>";
...
function excute(){
    global $pre_result,$result_cnt,$g5,$debug,$coin,$bonus_day;
    ...
    $update_shop_sql = "update {$g5['g5_shop_order_table']} set ";
    ...
    $update_sql = "update {$g5['member_table']} set ";
    ...
    for($i=0; $i < $row = sql_fetch_array($pre_result); $i++){
    ...
    if($column_sql != ""){
        ...
        echo "<span class='title block' style='font-size:30px;'>ESGC 스테이킹 발생자 : {$pay_token_member_count}</span>{$pay_token_list}<br><br>";
        echo "<span class='title block' style='font-size:30px;background:wheat;'>ETH 스테이킹 발생자 : {$pay_eth_member_count}</span>{$pay_eth_list}";
        ...
        if($debug){
            ...
            $result = sql_query($history_sql) ;
            if($result){
                $result = sql_query($update_shop_sql); 
                if($result){
