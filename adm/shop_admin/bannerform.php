...
include_once (G5_ADMIN_PATH.'/shop_admin/bannerset.php');
?>
<style type="text/css">
#bn_position {border:solid 1px #ccc;padding:8px;background-color:#f7f7f7;font-weight:bold;}
</style>
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
			...
			<?
				for ($i=0;$i<count($bnrs);$i++){
			?>
                <option value="<?=$bnrs[$i][1]?>" <?=($bnrs[$i][1] == $bn['bn_position'])?"selected":""?>><?=$bnrs[$i][0]?> <?=($bnrs[$i][2])?"[size. ".$bnrs[$i][2]."]":""?></option>			
			<?
				}
			?>
        </select>
