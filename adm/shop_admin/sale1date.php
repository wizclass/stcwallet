...
$g5['title'] = $fr_date.' ~ '.$to_date.' 일간 스테이킹 매출현황';
...
<form class="local_sch02 local_sch">
    <div class="sch_last">
        <input type="button" class="btn_submit excel" id="btnExport"  data-name='staking_saledate' value="엑셀 다운로드" />
    </div>
</form>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <thead>
    <tr>
        <th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=od_id&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">주문번호No<?php echo $ord_arrow[$ord_key]; ?></a></th>
        <th scope="col">아이디 / 이름</th>
        <th scope="col">상품명</th>
        <th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=orderprice&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">합계 (<?=ASSETS_CURENCY?>) <?php echo $ord_arrow[$ord_key]; ?></a></th>
        <th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=od_tax_flag&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">수수료 (<?=ASSETS_CURENCY?>)<?php echo $ord_arrow[$ord_key]; ?></a></th>
        <th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=od_cart_price&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">실 스테이킹 수량 (<?=ASSETS_CURENCY?>)<?php echo $ord_arrow[$ord_key]; ?></a></th>
        <!-- <th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=od_cash&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">현금가 (<?=BALANCE_CURENCY?>)<?php echo $ord_arrow[$ord_key]; ?></a></th> -->

		<th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=upstair&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">총지급예정 (<?=ASSETS_CURENCY?>)<?php echo $ord_arrow[$ord_key]; ?></a></th>
		<th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=pv&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">1회지급량 (<?=ASSETS_CURENCY?>)<?php echo $ord_arrow[$ord_key]; ?></a></th>
        <th scope="col">지급상태</th>
		<th scope="col"><a href="?ord=<?php echo $ord_rev; ?>&ord_word=od_invoice_time&fr_date=<?echo $fr_date;?>&to_date=<?=$to_date?>">만료일<?php echo $ord_arrow[$ord_key]; ?></a></th>
    </tr>
    </thead>
    <tbody>
    <?php
