...
function get_order_settle_sum($date)
{
    ...
    $case = array('매장카드','신용카드', '계좌이체', '가상계좌', '무통장', '휴대폰');
    $info = array();
...
/*##  ################################################*/
    ...
    $sql = " select sum(od_receipt_cash) as price,
                ...
                from {$g5['g5_shop_order_table']}
                where SUBSTRING(od_time, 1, 10) = '$date'
                  and od_receipt_cash > 0 ";
    $row = sql_fetch($sql);
    $info['현금']['price'] = (int)$row['price'];
    $info['현금']['count'] = (int)$row['cnt'];
/*@@End.  #####*/
    ...
    $row = sql_fetch($sql);
    $info['PV']['price'] = (int)$row['price'];
    $info['PV']['count'] = (int)$row['cnt'];
...
<section id="anc_sidx_settle">
    ...
    <div id="sidx_settle" class="tbl_head02 tbl_wrap">
        ...
        <tbody>
        <?php
        $case = array('현금','매장카드','신용카드', '계좌이체', '가상계좌', '무통장', '휴대폰');
		##$case = array('현금','매장카드','신용카드', '계좌이체', '가상계좌', '무통장', '휴대폰', 'PV', '쿠폰');
