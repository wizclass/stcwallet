...
$m_id = sql_fetch_array($result);
$mb_id = $m_id['mb_id'];

// 지불 금액
$sql_pay = "select payment_btc from {$g5['g5_shop_order_table']} where od_id ='{$od['od_id']}';";
$pay = sql_query($sql_pay);
$pay_coin = sql_fetch_array($pay);
$payment = $pay_coin['payment_btc'];

$sel_id = "select mb_wallet from g5_member where mb_id='$mb_id';";
$rst = sql_query($sel_id);
$w_rst = sql_fetch_array($rst);
$wallet_addr = $w_rst['mb_wallet'];
...
<section id="anc_sodr_list">
    ...
    <div class="local_desc02 local_desc">
        <p>
			...
			|
			입금 지갑주소<a href="#" onclick="window.open('https://blockchain.info/address/<?=$wallet_addr?>','width=800, height=500');"> <strong><?php echo $wallet_addr ?></strong></a> 
			| 인보이스 금액 <strong><?php echo $payment?> </strong>
        </p>
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
           ...
           $sql = " select ct_id, it_id, ct_price, ct_point, ct_qty, ct_option, ct_status, cp_price, ct_stock_use, ct_point_use, ct_send_cost, io_type, io_price
            ...
            for($k=0; $opt=sql_fetch_array($res); $k++) {
				...
				$bv = sql_fetch(" select it_bv from g5_shop_item where it_id = '{$opt['it_id']}' ");
                $ct_bv['stotal'] = $bv['it_bv'] * $opt['ct_qty'];
            ?>
            <tr>
                <?php if($k == 0) { ?>
                ...
                <td class="td_num"><?php echo number_format($ct_bv['stotal']); ?></td>
    ...
    <div class="btn_list02 btn_list">
        <p>
			...
			<input type="submit" name="ct_status" value="강제입금" onclick="document.pressed=this.value">
        </p>
    </div>
...
<section id="anc_sodr_pay">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col">배송비</th>
            <th scope="col" style="background-color:#FFE2E2;">현금결제</th>
        ...
        <tr>
            ...
            <td class="td_numbig" style="background-color:#FFE2E2;"><?php echo display_point($od['od_receipt_cash']); ?></td>
    ...
    <div class="compare_wrap">
        ...
        <section id="anc_sodr_chk" class="compare_left">
            ...
            <div class="tbl_frm01">
				...
				<tr>
					<th>현금결제 금액</th>
					<td><input type="text" name="od_receipt_cash" value="<?=$od['od_receipt_cash']?>" class="frm_input"/></td>
				</tr>
				<tr>
					<th>매장카드결제 금액 </th>
					<td><input type="text" name="od_receipt_price" value="<?=$amount['receipt']?>" class="frm_input"/></td>
				</tr>
				<? } else { ?>
                        ...
                        if ($od['od_settle_case'] != '무통장') {
                            switch($od['od_pg']) {
                                ...
                                case 'inicis':
                                    ...
                                    break;
                                case 'billgate':
                                    $pg_url  = 'https://www.billgate.net/partner/pt_franchise.jsp';
                                    $pg_test = '빌게이트';
                                    break;
                                case 'KAKAOPAY':
        ...
        <section id="anc_sodr_paymo" class="compare_right">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    <th scope="row"><label for="od_receipt_point">PV 결제액</label></th>
