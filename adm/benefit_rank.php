...
if($to_date==''){ 
	...
	function my_bchild_hap($mb_id){
		...
		echo "select count(mb_id) as bcnt from g5_member where mb_brecommend='".$mb_id."' order by mb_no";
		echo '<br>';
...
function habu_rank($recom){
    ...
    for ($j=0; $rrr=sql_fetch_array($res); $j++) { 
		...
		echo $id[$j].":".$one.$two.$three.$four.$five.$six."<br>";
...
for ($i=0; $row=sql_fetch_array($result); $i++) {   
	...
	echo "select (noo)as hap from bnoo where mb_id='".$mbid."'";
	echo "select (thirty)as hap from bthirty where mb_id='".$mbid."'";
	...
	if($twoleg==1){
		echo $mbname.'('.$mbid.')은 2레그 X 혹은 현재 직급: '.$mblevel.' 누적매출+자기매출: '.$noohap.' 30일간매출 : '.$thirtyhap.'<br>';
			...
			//3스타 조건
			if( ( ($noohap>=75000)  )  && ( ($thirtyhap>=15000) ) ){
			...
			//4스타 조건
			if( ( ($noohap>=350000) )  && ( ($thirtyhap>=70000)  ) ){
			...
			//5스타 조건
			if( ( ($noohap>=3500000)  )  && ( ($thirtyhap>=700000) ) ){
