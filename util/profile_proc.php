...
if ($category == 'pw') {
	...
	if (check_password($_POST['current_pw'], $member['mb_password'])) {
		...
		if (preg_match("/^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,16}$/", $_POST['new_pw']) == false) {
			echo (json_encode(array("result" => "error",  "code" => "0002", "sql" => '비밀번호는 최소 8자 최대 16자, 영문과 특수문자 숫자가 하나 이상 포함되어야 합니다.')));
			return false;
		}
...
if ($category == 'tpw') {
	...
	if (check_password($_POST['auth_pwd'], $member['mb_password'])) {
		...
		if (check_password($_POST['current_tpw'], $member['reg_tr_password'])) {
			...
			$pass_sql = " UPDATE g5_member set reg_tr_password = '{$get_encrypt_string}', reg_tr_password_change = 1 where mb_id = '{$member['mb_id']}' ";
			$pass_result = sql_query($pass_sql);
