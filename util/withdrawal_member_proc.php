...
$deposit_mb_id = trim($_POST['deposit_mb_id']);
$amt    = trim($_POST['amt']);
...
if ($rst) {
  ...
  $columns = "mb_shift_amt = mb_shift_amt + {$amt}";

  $amt_query = "UPDATE g5_member set 
	{$columns}
	, otp_key = ''
	where mb_id = '{$mb_id}' ";
  $deposit_query = "UPDATE g5_member set mb_deposit_point = mb_deposit_point + {$amt} where mb_id = '{$deposit_mb_id}'";
}
...
if ($debug) {
  ...
  $deposit_result = sql_query($deposit_query);
}
