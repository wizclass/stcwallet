...
function purchase_package($mb_id,$pack_id,$return = 0){
    ...
    global $g5,$debug,$now_date,$now_datetime,$od_id,$od_invoce_time;
    ...
    if(count($pack_target) > 0){
        ...
        for($i=0; $i < count($pack_target); $i++){
            ...
            $pack_result_sql = "INSERT {$pack_table} SET mb_id = '{$mb_id}', idx= {$count_colum_cnt},it_name='{$it_name}', nth = {$count_item_cnt}, pdate='{$od_invoce_time}', cdate = curdate(), cdatetime = now() , od_id = {$od_id}, it_id = '{$pack_id}' ";
        ...
        if($result && $proc_done && $return == 0  ){
            ...
            ob_end_clean();
            echo json_encode(array("response"=>"failed","code" => '0001', "data"=>$pack_result_sql),JSON_UNESCAPED_UNICODE);
        }
    }
}
