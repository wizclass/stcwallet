...
if( $cert_enc_use == "Y" )
{
    ...
    if( $res_cd == "0000" )
    {
        ...
        // 폰번호로 찾도록 커스텀 수정 _ BY SOO
        $row = sql_fetch("select mb_id from {$g5['member_table']} where mb_hp = '{$phone_no}' "); 
        if (empty($row['mb_id'])){
...
<form name="mbFindForm" method="POST">
    ...
    <div class="result_wrap mt-3">
        ...
        <p class="find_id"><?php echo $row["mb_id"]; ?></p>
    </div>
