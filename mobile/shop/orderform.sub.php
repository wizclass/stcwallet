...
function pay_approval()
{
    ...
    <?php if($default['de_pg_service'] == 'billgate') { ?>
    switch(settle_method)
    {
        case "계좌이체":
            f.PAY_METHOD.value = "account";
            f.action = "http://tpay.billgate.net/account/smartphone/certify.jsp";   //test
            //f.action = "https://pay.billgate.net/account/smartphone/certify.jsp"; //real
            $('#RETURN_URL').val('http://<?=$_SERVER['HTTP_HOST']?>/mobile/shop/billgate/return.php');
            //상용 URL 
            //f.action = "https://pay.billgate.net/account/certify.jsp";
            break;
        case "휴대폰":
            f.PAY_METHOD.value = "phone";
            f.action = "http://tpay.billgate.net/mobile/smartphone/certify.jsp" ;   //test
            //f.action = "https://pay.billgate.net/mobile/smartphone/certify.jsp" ;	
            $('#RETURN_URL').val('http://<?=$_SERVER['HTTP_HOST']?>/mobile/shop/billgate/return_phone.php');
            //f.action = "https://pay.billgate.net/mobile/certify.jsp" ; //REAL
            break;
        case "신용카드":
            f.PAY_METHOD.value = "card";
        
            f.action = 'http://tpay.billgate.net/credit/smartphone/certify.jsp';  //test
            //f.action = 'https://pay.billgate.net/credit/smartphone/certify.jsp';  //real
            $('#RETURN_URL').val('http://<?=$_SERVER['HTTP_HOST']?>/mobile/shop/billgate/return.php');
            //f.action = "https://pay.billgate.net/credit/certify.jsp";
            break;
        default:
    ...
    f.AMOUNT.value = f.good_mny.value;
    f.USER_NAME.value = pf.od_name.value;
    //f.ITEM_NAME.value = encodeURIComponent('테스트');
    if(f.PAY_METHOD.value != "무통장") {

        var param = "od_id="+f.ORDER_ID.value+"&amount="+f.good_mny.value;
        var check_sum = "";
        $.ajax({
            type: "POST",
            data: param,
            url: g5_url+"/shop/billgate/ajax_checksum.php",
            cache: false,
            async: false,
            success: function(data) {
                check_sum = data;
                f.CHECK_SUM.value = check_sum;
            }
        });

	    f.target = "payment";
        var option ="width=550,height=600,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,left=150,top=150";
