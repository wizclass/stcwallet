	...
	$(function() {
		...
		$('#ticket').on('click', function(e) {
			...
			if($('#subject').val() != '' && $('#content').val() != ''){
				...
				// commonModal('문의 알림','<i class="fas fa-exclamation-triangle red"><h4><br>문의하실 내용을 적어주세요.</h4>',80);
				dialogModal("문의 알림", "문의하실 내용을 적어주세요.", 'warning');
			}
		});
				...
				<div class="support-container">
					<div class="support-panels">
						<ul class="support-tabs">
							<li rel="open-new-ticket" class="active" ><span>새 티켓 열기</span></li>
							<li rel="active-tickets" ><span>활성화 티켓</span></li>
							<li rel="closed-tickets" ><span>비활성화 티켓</span></li>
						</ul>
						<div id="open-new-ticket" class="panel box_ty01 active">
							<form id="ticketForm" action ="/util/support_center.ticket.php" method="post" enctype="multipart/form-data" >
								<input type="hidden" name="lang">
								<div class="input-group mb-3">
									<div class="input-group-prepend">
										<label class="input-group-text" for="topic">주제선택</label>
									</div>
								...
								<div class="submit-button">
									<div class="btn wd main_btn font_white" id="ticket"> 티켓제출</div>
								</div>
							</form>
						</div>
						<div id="active-tickets" class="panel container"></div>
						<div id="closed-tickets" class="panel container"></div>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	if(url.indexOf('support_center') != -1) {
